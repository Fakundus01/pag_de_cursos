import { CheckCircle2, ChevronLeft, Home, LockKeyhole, ShieldCheck, Sparkles, Star } from "lucide-react";
...
export const CoursePlayerPage = () => {
  ...
  useEffect(() => {
    return () => {
      if (completionRedirectRef.current !== null) {
        ...
        <div className="course-complete-overlay">
          <div className="course-complete-card text-center">
            ...
            <p className="relative z-10 mt-4 max-w-md text-sm leading-7 text-steel">
              Terminaste todas las secciones. El progreso ya quedo guardado y te mandamos al inicio para que veas la ruta marcada como hecha.
            </p>
            <div className="relative z-10 mt-6 inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/5 px-4 py-2 text-sm text-white">
              <Home size={16} />
              Volviendo al inicio...
            </div>
          </div>
                  ...
                  return (
                    <div key={section.id} className="rounded-[24px] border border-white/10 bg-abyss/60 p-5">
                      <div className="flex flex-wrap items-center justify-between gap-4">
                        ...
                        <button
                          type="button"
                          disabled={savingSectionId === section.id || isCompleted || !canAccessPremium || courseCelebration}
                          ...
                          onClick={async () => {
                            ...
                            const currentCompletedCount = course.sections.filter((item) => item.completed || completedSections.has(item.id)).length;
                            const finishedCourse = course.sections.length > 0 && currentCompletedCount + (isCompleted ? 0 : 1) >= course.sections.length;
                            ...
                            if (finishedCourse) {
                              setFeedback("Curso completado. Preparando el regreso al inicio...");
                            ...
                            <div key={content.id} className="rounded-2xl border border-white/10 bg-white/5 p-4 text-sm">
                              <div className="flex items-center justify-between gap-3">
                                <div>
                                  ...
                                  <p className="mt-1 text-xs uppercase tracking-[0.18em] text-aurora">
                                    {content.type} · {content.estimatedMinutes} min
                                  </p>
                                </div>
                                {isVisible ? (
